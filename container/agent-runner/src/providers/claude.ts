...
export class ClaudeProvider implements AgentProvider {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    // container config (`this.inference.model`) wins; CLAUDE_MODEL env is the
    // fallback; 'sonnet' is the final default. CLAUDE_MODEL must be in
    // CONTAINER_PASSTHROUGH in .env to forward from the host.
    const model = this.inference.model || process.env.CLAUDE_MODEL || 'sonnet';
