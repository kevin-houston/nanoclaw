...
export function resolveClaudeInference(
  ...
  // container config wins; CLAUDE_MODEL env is the fallback; 'sonnet' is the
  // final default. CLAUDE_MODEL must be in CONTAINER_PASSTHROUGH in .env to
  // forward from the host.
  return {
    model: input.model || environment.CLAUDE_MODEL || 'sonnet',
    effort: input.effort,
