...
export async function buildMounts(
  ...
  // Privileged mounts — explicitly bypass the mount-security allowlist so they
  // land at their specified absolute container paths (e.g. /var/run/docker.sock).
  ...
  // Classed 'allowlisted-extra': the operator-configured read-write class, the
  // only one permitted unconditionally by validateSpec. Paths under the pinned
  // materials/surface roots still get their required class and are refused here.
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  // Ollama admin tools flag (add-ollama-tool skill) — a plain boolean, so it
  // rides the composed lane.
  if (OLLAMA_ADMIN_TOOLS) env.OLLAMA_ADMIN_TOOLS = 'true';
  ...
  // from the credential-NAME check. Credential VALUES are still refused there —
  // an issuer-prefixed secret (e.g. `sk-…`) belongs in the vault, not here.
  if (CONTAINER_ENV_PASSTHROUGH.length > 0) {
    ...
    for (const key of CONTAINER_ENV_PASSTHROUGH) {
      const value = passthroughValues[key];
      if (value) contributedEnv[key] = value;
    }
  }
