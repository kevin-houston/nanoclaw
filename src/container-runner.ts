...
export async function buildMounts(
  ...
  // Privileged mounts — land at their specified absolute container paths
  // (e.g. /var/run/docker.sock) rather than under the /workspace/extra/ prefix
  // the allowlist imposes. Classed 'allowlisted-extra': the operator opts in
  // explicitly per group via container.json / `ncl groups config update`, which
  // is the same vetted-upstream contract provider mounts ride.
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  // Non-secret feature flag: lets the Ollama MCP tool expose model-management
  // verbs. Plain composed env — no credential shape, so no admission concern.
