...
describe('buildMounts against the policy the drivers enforce', () => {
  ...
  /**
   * Local customization: `privilegedMounts` land at an absolute container path
   * of the operator's choosing (`/var/run/docker.sock`), bypassing the
   * `/workspace/extra/` prefix the allowlist imposes. Under the driver seam
   * they must still survive admission, so this pins the class they are composed
   * with — `allowlisted-extra`, the same vetted-upstream contract provider
   * mounts ride. A demotion to any pinned class denies every session for the
   * group that opted in.
   */
  it('carries a privileged mount through admission at its absolute target', async () => {
    const withPrivileged = {
      ...containerConfig,
      privilegedMounts: [{ hostPath: '/var/run/docker.sock', containerPath: '/var/run/docker.sock' }],
    } as unknown as ContainerConfig;
    const mounts = await buildMounts(agentGroup, session, withPrivileged, 'claude', {});
    const sock = mounts.find((m) => m.containerPath === '/var/run/docker.sock');
    expect(sock).toMatchObject({ mountClass: 'allowlisted-extra', readonly: false });
